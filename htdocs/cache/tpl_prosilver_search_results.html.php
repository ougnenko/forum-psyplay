...
<?php } if ($this->_rootref['S_SHOW_TOPICS']) {  if (sizeof($this->_tpldata['searchresults'])) {  ?>
	...
	<div class="forumbg">
		...
		<?php $_searchresults_count = (isset($this->_tpldata['searchresults'])) ? sizeof($this->_tpldata['searchresults']) : 0;if ($_searchresults_count) {for ($_searchresults_i = 0; $_searchresults_i < $_searchresults_count; ++$_searchresults_i){$_searchresults_val = &$this->_tpldata['searchresults'][$_searchresults_i]; if ($_searchresults_val['S_DELETED']) {  ?>

			<li class="row<?php if (!($_searchresults_val['S_ROW_COUNT'] & 1)  ) {  ?> bg1<?php } else { ?> bg2<?php } if ($_searchresults_val['S_POST_ANNOUNCE']) {  ?> announce<?php } if ($_searchresults_val['S_POST_STICKY']) {  ?> sticky<?php } if ($_searchresults_val['S_TOPIC_REPORTED']) {  ?> reported<?php } ?> deleted_topic">
				<dl class="icon" style="background-image: url(<?php echo $_searchresults_val['TOPIC_FOLDER_IMG_SRC']; ?>); background-repeat: no-repeat;">
					<dt style="<?php if ($_searchresults_val['TOPIC_ICON_IMG'] && $this->_rootref['S_TOPIC_ICONS']) {  ?>background-image: url(<?php echo (isset($this->_rootref['T_ICONS_PATH'])) ? $this->_rootref['T_ICONS_PATH'] : ''; echo $_searchresults_val['TOPIC_ICON_IMG']; ?>); background-repeat: no-repeat;<?php } ?>" title="<?php echo $_searchresults_val['TOPIC_FOLDER_IMG_ALT']; ?>"><?php if ($_searchresults_val['S_UNREAD_TOPIC']) {  ?><a href="<?php echo $_searchresults_val['U_NEWEST_POST']; ?>"><?php echo (isset($this->_rootref['NEWEST_POST_IMG'])) ? $this->_rootref['NEWEST_POST_IMG'] : ''; ?></a> <?php } ?><a href="<?php echo $_searchresults_val['U_VIEW_TOPIC']; ?>" class="topictitle"><?php echo $_searchresults_val['TOPIC_TITLE']; ?></a>
						...
						<?php if ($_searchresults_val['PAGINATION']) {  ?><strong class="pagination"><span><?php echo $_searchresults_val['PAGINATION']; ?></span></strong><?php } if ($_searchresults_val['ATTACH_ICON_IMG']) {  echo $_searchresults_val['ATTACH_ICON_IMG']; ?> <?php } ?>

						<?php echo $_searchresults_val['DELETED_MSG']; ?>
	...
	<?php } } else { $_searchresults_count = (isset($this->_tpldata['searchresults'])) ? sizeof($this->_tpldata['searchresults']) : 0;if ($_searchresults_count) {for ($_searchresults_i = 0; $_searchresults_i < $_searchresults_count; ++$_searchresults_i){$_searchresults_val = &$this->_tpldata['searchresults'][$_searchresults_i]; ?>
		...
		<div class="search post <?php if (($_searchresults_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } if ($_searchresults_val['S_POST_REPORTED']) {  ?> reported<?php } if ($_searchresults_val['S_DELETED']) {  ?> deleted_post<?php } ?>">
			<div class="inner"><span class="corners-top"><span></span></span>
