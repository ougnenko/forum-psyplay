	...
	<?php } else { if ($_forumrow_val['S_NO_CAT']) {  ?>
		...
		<tr>
			...
			<td class="row1" width="100%">
				...
				<?php } $_reput_count = (isset($_forumrow_val['reput'])) ? sizeof($_forumrow_val['reput']) : 0;if ($_reput_count) {for ($_reput_i = 0; $_reput_i < $_reput_count; ++$_reput_i){$_reput_val = &$_forumrow_val['reput'][$_reput_i]; if ($_reput_val['S_THANKS_FORUM_REPUT_VIEW'] && $_reput_val['FORUM_REPUT'] && ! $this->_rootref['S_IS_BOT']) {  ?>
					...
					<p class="forumdesc" style="clear: left"><strong><?php echo $_forumrow_val['L_SUBFORUM_STR']; ?></strong> <?php echo $_forumrow_val['SUBFORUMS']; ?></p>
				<?php } ?>
			...
			<td class="row2" align="center" nowrap="nowrap">
				<?php if ($_forumrow_val['LAST_POST_TIME']) {  if ($this->_rootref['S_ALTT_ACTIVE']) {  ?><p><?php if ($_forumrow_val['ALTT_LINK_NAME_SHORT'] != ('')) {  ?><a <?php if ($this->_rootref['ALTT_STYLE']) {  ?>style="<?php echo (isset($this->_rootref['ALTT_STYLE'])) ? $this->_rootref['ALTT_STYLE'] : ''; ?>"<?php } ?> href="<?php echo $_forumrow_val['U_ALTT_LINK']; ?>" title="<?php echo $_forumrow_val['ALTT_LINK_NAME']; ?>"><?php echo $_forumrow_val['ALTT_LINK_NAME_SHORT']; ?></a><?php } else { echo ((isset($this->_rootref['L_ALTT_PROTECTED'])) ? $this->_rootref['L_ALTT_PROTECTED'] : ((isset($user->lang['ALTT_PROTECTED'])) ? $user->lang['ALTT_PROTECTED'] : '{ ALTT_PROTECTED }')); } ?></p><?php } ?>
